...
interface ColumnDef {
  ...
  width?: string
  editable?: boolean
  render?: (val: unknown, row: Candidate) => React.ReactNode
  editRender?: (
    val: unknown,
    row: Candidate,
...
const COLUMNS: ColumnDef[] = [
  ...
  { key: "is_fresh_grad", label: "是否为应届生", editable: true },
  { key: "channel", label: "招聘渠道", editable: true },
  { key: "recommend_detail", label: "推荐沟通情况", editable: true },
  { key: "department", label: "备注（特殊情况）", editable: true },
]
...
const RECOMMEND_TEMPLATE = `1、基本信息：籍贯、目前居住地、通勤距离、可到岗时间、是否在职（在职-考虑新机会原因；离职-离职原因）女性关注一下婚育问题；
2、求职动机&意向度（稳定性）：目前投递or拿到Offer的行业&岗位、选工作看重的点&评估的维度、对焦点的了解(重点看对销售岗位的意向）；
3、工作经验&职业规划：是否有相关销售经验（过往业绩考核模式&排名）、职业规划路径（方向是否明确）；
4、综合素质：沟通表达能力、逻辑性、抗压能力、目标感；
5、接受度：工作模式（B端电销）、薪资待遇（底薪+提成）、工作强度（加班、沉淀期）`
...
// ─── Component ───────────────────────────────────────────────────────
export default function RecommendationLedgerPage() {
  ...
  const startEdit = (c: Candidate) => {
    ...
    setEditForm({
      ...
      channel: c.channel ?? "",
      recommend_detail: c.recommend_detail ?? "",
  ...
  // Template pre-fill for recommend_detail
  const fillTemplate = () => {
    setEditForm((prev) => ({ ...prev, recommend_detail: RECOMMEND_TEMPLATE }))
  }

  // Move candidate to elimination
  const handleMoveToElimination = async (id: number, iname: string) => {
    if (!confirm("确定将该候选人转入淘汰台账吗？")) return
    try {
      await updateCandidate(id, { result: "淘汰" }, iname)
      setCandidates((prev) => prev.filter((c) => !(c.id === id && c.intern_name === iname)))
      setTotal((prev) => prev - 1)
      showToast("success", "已转入淘汰台账")
    } catch (err: any) {
  ...
  const copyRow = (row: Candidate, idx: number) => {
    const values = COLUMNS.map((col) => {
      ...
      const raw = formatCellValue(getCellValue(row, col.key))
  ...
  const exportXLSX = async () => {
    try {
      ...
      let p = 1
      while (true) {
        const params: Record<string, string | number> = { result: "推荐", page: p, page_size: 200 }
        if (filterUser) params.intern_name = filterUser
        if (dateStart) params.date_start = dateStart
        if (dateEnd) params.date_end = dateEnd
        const data = await getCandidates(params)
      ...
      XLSX.utils.book_append_sheet(wb, ws, "推荐台账")
      const d = new Date()
      const ds = `${d.getFullYear()}/${String(d.getMonth()+1).padStart(2,"0")}/${String(d.getDate()).padStart(2,"0")}`
      XLSX.writeFile(wb, `推荐台账_${ds}.xlsx`)
    ...
    <div className="min-h-screen bg-[#F6F8FB]">
      ...
      <main className="max-w-[1600px] mx-auto px-4 xl:px-6 py-6 xl:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h1 className="text-2xl font-bold text-[#152033]">推荐台账</h1>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#16A37B] text-white rounded-md text-sm font-medium hover:bg-[#128a67] disabled:opacity-50 transition-colors"
            >
        ...
        <div className="bg-white rounded-xl border border-[#E6EAF2] overflow-hidden">
          ...
          {!loading && !error && candidates.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-[#637089]">暂无推荐候选人数据</p>
            </div>
          )}
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          ...
                          isEditing ? "bg-[#F0F4FF]" : "hover:bg-[#F6F8FB]",
                        )}
                      >
                        {/* Index */}
                          ...
                          (col) => {
                                  ...
                                  {col.key === "recommend_detail" ? (
                                    <div className="flex items-center gap-1">
                                      <input
                                        ...
                                        className="w-full min-w-[120px] px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF]"
                                      />
                                      <div className="flex gap-0.5 shrink-0">
                                        <button
                                          onClick={fillTemplate}
                                          className="p-1 rounded hover:bg-[#EDF0F7] text-[#2F6BFF]"
                                          title="填充模板"
                                        >
                                        ...
                                        <button
                                          onClick={() =>
                                            setExpandField({
                                              id: row.id,
                                              key: "recommend_detail",
                                              value:
                                                editForm.recommend_detail ??
                                                "",
                                            })
                                          }
                                          className="p-1 rounded hover:bg-[#EDF0F7] text-[#637089]"
                                          title="展开编辑"
                                        >
                                    ...
                                    <select
                                      value={formatCellValue(
                                        editForm[col.key as keyof Candidate],
                                      )}
                                      onChange={(e) =>
                                        handleFieldChange(
                                          col.key as string,
                                      ...
                                      className="px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF] bg-white min-w-[70px]"
                                    >
                                    ...
                                    <select
                                      ...
                                      className="px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF] bg-white min-w-[90px]"
                                    >
                                      <option value="">请选择</option>
                                    ...
                                    <input
                                      ...
                                      className="px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF] bg-white min-w-[110px]"
                                    />
                                  ) : col.key === "position" ? (
                                    <select
                                      value={formatCellValue(
                                    ...
                                    <input
                                      ...
                                      className="w-20 px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF]"
                                      placeholder="年份"
                                    />
                                  ) : col.key === "channel" ? (
                                    <select
                                      ...
                                      className="px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF] bg-white min-w-[100px]"
                                    >
                                      <option value="">请选择</option>
                                    ...
                                    <input
                                      ...
                                      value={formatCellValue(editForm[col.key as keyof Candidate])}
                                      onChange={(e) => handleFieldChange(col.key as string, e.target.value)}
                                      className="px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF]"
                                    />
                                  ) : (
                                    <input
                                      type="text"
                                      ...
                                      className="w-full min-w-[80px] px-2 py-1 rounded border border-[#2F6BFF] text-sm focus:outline-none focus:ring-1 focus:ring-[#2F6BFF]"
                                    />
                                  )}
                                </td>
                              )
                            ...
                            return (
                              <td
                                ...
                                title={col.key === "communicate_time" ? formatCellValue(val).replace(/-/g, "/") : formatCellValue(val)}
                              >
                                {col.key === "communicate_time" ? formatCellValue(val).replace(/-/g, "/") : formatCellValue(val)}
                              </td>
                            )
                          },
                        )}
                          ...
                          <div className="flex items-center justify-center gap-1">
                            {isEditing ? (
                              ...
                              <>
                                ...
                                <button
                                  onClick={() => handleMoveToElimination(row.id, row.intern_name)}
        ...
        {expandField && (
          ...
          >
            <div
              className="bg-white rounded-xl shadow-xl border border-[#E6EAF2] p-6 w-full max-w-lg mx-4"
            ...
            >
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  handleFieldChange("recommend_detail", e.target.value)
                }}
                className="w-full min-h-[200px] px-3 py-2 rounded-md border border-[#C8CDD8] text-sm focus:outline-none focus:ring-2 focus:ring-[#2F6BFF] resize-y"
                placeholder="请输入推荐沟通情况..."
              />
              <div className="flex justify-between items-center mt-3">
                <button
                  onClick={() => {
                    setEditForm((prev) => ({
                      ...prev,
                      recommend_detail: RECOMMEND_TEMPLATE,
                    }))
                    setExpandField((prev) =>
                      prev
                        ? { ...prev, value: RECOMMEND_TEMPLATE }
                        : null,
                    )
                  }}
                  className="text-sm text-[#2F6BFF] hover:underline"
                ...
                <button
                  onClick={() => setExpandField(null)}
                  className="px-4 py-2 bg-[#2F6BFF] text-white rounded-md text-sm font-medium hover:bg-[#1a5ae0]"
                >
                  确定
                </button>
              </div>
